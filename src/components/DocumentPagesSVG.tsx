...
import usePageToSVG from '~/hooks/usePageToSVG';

let lastZoom = 1;
const DocumentPagesSVG: React.FC<{ currentPage: number }> = ({ currentPage }) => {
  const container = useRef<HTMLDivElement>(null);
  const [scale, setScale] = useState<number>(lastZoom);
  const [embedFonts, setEmbedFonts] = useState<boolean>(false);
  const svg = usePageToSVG(currentPage, embedFonts);
  useEffect(() => {
...
export default DocumentPagesSVG;
