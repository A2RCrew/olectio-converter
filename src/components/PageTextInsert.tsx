...
import useCustomInsert from '~/hooks/useCustomInsert';

const PageTextInsert: React.FC<{ currentPage: number }> = ({ currentPage }) => {
  const [text, setText] = usePageText(currentPage);
  const  [response, correct] = useCustomInsert(text);
        ...
        {response ? (
          ...
          <p className="text-center">
            <button onClick={correct} className="bg-pink-500 p-2 px-4 mx-4 mt-10 rounded-full">
              Insert Text in the [insert] placeholder
            </button>
          </p>
        )}
      </div>
